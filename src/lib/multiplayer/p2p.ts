...
export interface P2PRoomOptions {
  ...
  selfId: string;
  name?: string;
  /** Defaults to same-origin `/api/rtc`. Pass an absolute URL for a closed net. */
  signalingUrl?: string;
  /** Defaults to VITE_STUN_URLS (comma-separated) or Google public STUN. */
  iceServers?: RTCIceServer[];
  /** Optional mic/camera stream. Audio tracks start disabled (push-to-talk). */
...
export class P2PRoom {
  ...
  private endpoint(query?: URLSearchParams): string {
    if (!query) return this.signalingUrl;
    const join = this.signalingUrl.includes("?") ? "&" : "?";
    return `${this.signalingUrl}${join}${query.toString()}`;
  }
  ...
  close(): void {
    ...
    const leftover = [...this.peers.keys()];
    this.peers.clear();
    for (const id of leftover) this.opts.onRemoteStreamRemoved?.(id);
  ...
  /** Enable or disable outbound audio (push-to-talk). */
  setAudioEnabled(enabled: boolean): void {
    if (!this.localStream) return;
    for (const track of this.localStream.getAudioTracks()) track.enabled = enabled;
  }

  /** Attach or replace the local media stream on every live peer connection. */
  setLocalStream(stream: MediaStream | null): void {
    this.localStream = stream;
    for (const slot of this.peers.values()) this.syncAudioTrack(slot);
  }
  ...
  private reconcileRoster(peers: { id: string; name: string }[]): void {
    ...
    for (const [id, slot] of this.peers) {
      if (!alive.has(id)) {
        ...
        this.opts.onRemoteStreamRemoved?.(id);
      }
    }
    this.emitPeers();
  }
  ...
  private connectTo(peerId: string, name: string, initiator: boolean): PeerSlot | null {
    ...
    pc.ontrack = (e) => {
      ...
      e.track.addEventListener("ended", () => this.opts.onRemoteStreamRemoved?.(peerId));
      this.opts.onRemoteStream?.(peerId, stream);
    };

    this.syncAudioTrack(slot);
  ...
  private syncAudioTrack(slot: PeerSlot): void {
    const track = this.localStream?.getAudioTracks()[0] ?? null;
    const sender = slot.pc.getSenders().find((s) => s.track?.kind === "audio");
    if (sender) {
