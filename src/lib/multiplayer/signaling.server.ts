 ...
 * mesh forms; audio then flows peer-to-peer. CORS is open so a Closed-net
 * Squelch client can point at this instance as its server.
 */
import { z } from "zod";
...
const signalSchema = z.object({
  ...
  payload: z.unknown().refine((v) => isValidSignalPayload(v), {
...
async function handleGet(url: URL): Promise<Response> {
  ...
  const current = await roster(sql, room);
  const others = current.filter((p) => p.id !== peer);
  if (others.length >= MAX_PEERS_PER_CHANNEL) {
    return json({ error: "channel full", peers: current, signals: [] }, 409);
  }
...
async function handlePost(request: Request): Promise<Response> {
  ...
  if (msg.op === "signal") {
    if (!isValidSignalId(msg.from) || !isValidSignalId(msg.to)) {
...
export async function handleChannelList(request: Request): Promise<Response> {
  try {
    if (request.method === "OPTIONS") {
      return new Response(null, { status: 204, headers: corsHeaders() });
    }
    ...
    const sql = await getSql();
    await ensureSchema(sql);
    const rows = await sql.query<{ room: string; listeners: number }>(
      `SELECT room, COUNT(*)::int AS listeners
       FROM webrtc_peers
       WHERE last_seen > now() - make_interval(secs => $1)
       GROUP BY room
       ORDER BY listeners DESC, room
       LIMIT 50`,
      [PEER_TTL_SECONDS],
    );
    return json({
      channels: rows.map((r) => ({ id: r.room, listeners: r.listeners })),
    });
  } catch (error) {
    console.error("[rtc] channel list error:", error);
    return json({ error: "channel list failed" }, 500);
  }
}
